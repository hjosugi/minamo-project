...
日付: 2026-07-05（初回パス、v0.1.4）。v0.1.5-v0.1.12 は下の追記を参照。
...
この台帳は v0.1.4 時点で書かれ、その後のリリースに合わせて日付が更新されていま
せんでした。元のパスを書き直すのではなく、それ以降の主な変更をここに記録します
(#266):

- **v0.1.7** ネイティブアバターの起動: デスクトップシェルが Tauri ブリッジ
  (`pick_native_avatar` / `read_native_avatar`) 経由で VRM を開き、ビューアが
  描画します。
- **v0.1.8** バイリンガルドキュメント体制: すべてのドキュメントに `.ja.md` の対
  があり、`scripts/verify_structure.py` が対応を検証します。
- **v0.1.9 - v0.1.12** リレーとデスクトップのセキュリティ強化 (#246、#247、
  #251)、トラッカー／リプレイのホットパスにおけるフレームごとの割り当て削減
  (#259)、共有ルームでの参加者ごとの決定的なアバター割り当て (#225)、全プロダ
  クトUIのランタイム EN/JA ローカライズ (#267)。
- **v0.1.12 以降** テストとプロトコルの整備: ページのスモークテストと純粋ロジッ
  クの抽出、CIカバレッジゲート (#263)、非信頼経路のバイナリパーサに対するプロパ
  ティテスト (#262)、KGM1B のバージョンゲートと言語間コンフォーマンスベクタ
  (#256、#257)、および本バックログの整合 (#266)。

台帳自体の位置づけは変わりません。リポジトリ上の証跡を記録するものであり、Issue
のクローズを意味しません。Issue は各自の受け入れ基準が満たされるまで開いたまま
です。

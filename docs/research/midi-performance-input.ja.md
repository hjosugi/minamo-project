...
> **訂正 (#240): この文言は Safari には正しく、Firefox には誤りである。**
> Firefox は Web MIDI を実装しているが、`requestMIDIAccess()` はユーザーが生成された *サイト権限アドオン* を
> インストールするまで **常に reject する**（localhost は例外）。機能検出は通過し、それでも Promise は失敗するため、
> 「このブラウザでは利用できない」はそこでは虚偽であり、かつ行動につながらない。API の存在と「ここで API が resolve する」は
> 別の事実であり、UI はそれらを別物として扱う必要がある —
> [ble-midi-drum-sticks.md](ble-midi-drum-sticks.md) と `selectStickTransport` を参照。
